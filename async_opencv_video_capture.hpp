 ...
 *     ocv::camera::async_opencv_video_capture<> cl([&](std::exception const
 * &ex)
 *     {
 *         //cerr of c++ is not a thread safe class, so we need to lock the
 * mutex std::unique_lock<std::mutex> lock(emutex); std::cerr<<"camera
 * exception:"<<ex.what()<<std::endl; return true;
 *     }, wait_msec);
 *     cl.open_url(argv[1]);
 *
 ...
 * @endcode
 */
template <typename Mutex = std::mutex> class async_opencv_video_capture {
  public:
    using listener_key = void const *;

    /**
     * @param cam_exception_listener a listener to process exception if the
     * video capture throw exception
     * @param wait_msec Determine how many milliseconds the videoCapture will
     * wait before capture next frame
     * @param replay Restart the camera if the frame is empty
     * @warning If mutex type is std::mutex, unless the listener do not run in
     * the same thread of videoCapture, else do not call the api of
     * async_opencv_video_capture in the listener, this may cause dead lock. Use
     * std::recursive_mutex if you want to do that
     */
    explicit async_opencv_video_capture(
        std::function<bool(std::exception const &ex)> cam_exception_listener,
        long long wait_msec = 0, bool replay = true);
     ...
     * 2. If mutex type is std::mutex, unless the listener do not run in the
     * same thread of videoCapture, else do not call the api of
     * async_opencv_video_capture in the listener, this may cause dead lock. Use
     * std::recursive_mutex if you want to do that
     */
            ...
            for (;;) {
                ...
                if (!stop_ && !listeners_.empty()) {
                    try {
                        ...
                        // reopen the camera if exception thrown ,this may
                        // happen frequently when you receive frames from network
                        cap_.open(url_);
...
async_opencv_video_capture<Mutex>::async_opencv_video_capture(
    std::function<bool(const std::exception &)> cam_exception_listener,
    long long wait_msec, bool replay)
    : cam_exception_listener_(std::move(cam_exception_listener)),
      replay_(replay), stop_(false),
...
template <typename Mutex>
void async_opencv_video_capture<Mutex>::remove_listener(
    async_opencv_video_capture::listener_key key) {
    std::lock_guard<Mutex> lock(mutex_);
    auto it = std::find_if(std::begin(listeners_), std::end(listeners_),
                           [&](auto const &val) { return val.first == key; });
