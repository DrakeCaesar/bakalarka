...
This class implements a very fast ellipse detector, codename: YAED (Yet Another
Ellipse Detector)
...
class CEllipseDetectorYaed {
    ...
    Size _szPreProcessingGaussKernelSize; // size of the Gaussian filter in
                                          // preprocessing step
    double _dPreProcessingGaussSigma;     // sigma of the Gaussian filter in the
                                          // preprocessing step

    // Selection strategy - Step 1 - Discard noisy or straight arcs. See Sect []
    // in the paper
    int _iMinEdgeLength;         // minimum edge size
    float _fMinOrientedRectSide; // minumum size of the oriented bounding box
                                 // containing the arc
    float _fMaxRectAxesRatio; // maximum aspect ratio of the oriented bounding
                              // box containing the arc
    ...
    float _fMaxCenterDistance;  // maximum distance in pixel between 2 center
                                // points
    ...
    float
        _fDistanceToEllipseContour; // maximum distance between a point and the
                                    // contour. See equation [] in the paper
    ...
    vector<double> _times; // _times is a vector containing the execution time
                           // of each step. _times[0] : time for edge detection
  ...
  public:
    ...
    void DetectAfterPreProcessing(vector<Ellipse> &ellipses, Mat1b &E,
                                  const Mat1f &PHI = Mat1f());
    ...
    double GetExecTime() {
        return _times[0] + _times[1] + _times[2] + _times[3] + _times[4] +
               _times[5];
    }
  ...
  private:
    ...
    float GetMedianSlope(vector<Point2f> &med, Point2f &M,
                         vector<float> &slopes);
    ...
    {
        _times[idx] = ((double)cv::getTickCount() - _times[idx]) * 1000. /
                      cv::getTickFrequency();
