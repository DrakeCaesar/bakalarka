...
void GrabThread(VideoCapture *cap) {
    ...
    // To know how many memory blocks will be allocated to store frames in the
    // queue. Even if you grab N frames and create N x Mat in the queue only few
    // real memory blocks will be allocated thanks to std::queue and cv::Mat
    // memory recycling
    ...
    std::cout << std::endl
              << "Number of Mat in memory: " << matMemoryCounter.size();
}
int duration = 0;
int duration1 = 0;
...
int main() {
    ...
    // Draw axis
    Mat Eye_Waveform =
        Mat::zeros(900, 900, CV_8UC3); // Waveform image used to record blinks
    Point p1 = Point(10, 0);
    ...
    // Each blink EAR has to go through the process from greater than 0.2-less
    // than 0.2-greater than 0.2
    ...
    try {
        ...
        for (int j=0; j < frames; j = j+8) {
            std::thread t[8];
            for (int i = 0; i < 8; i++) {
                t[i] = std::thread(processWithDlib, &(data[j+i]),
        &(faces[j+i]));
                //if (j % 100 == 0) {
        ...
        */
        ...
        for (int k = 0; k < frames - 1; k++) {
            ...
            // "wink_masked/img-%d_surgical.png",k+1);
            ...
            if (load == false) {
                ...
                unsigned int faceNumber =
                    faces[k].size(); // Get the number of vectors in the
                                     // container, that is, the number of faces
            ...
            if (!shapes.empty()) {
                ...
                for (int j = 0; j < faceNumber; j++) {
                    ...
                    if (load == false) {
                        char fileName[64];
                        stringstream fileNameStringStream;
                        fileNameStringStream << "facialData/" << k << "." << j
                                             << ".txt";
                    ...
                    for (int i = 0; i < 68; i++) {
                        ...
                        if (load == false) {
                            cv::circle(temp,
                                       cvPoint(shapes[j].part(i).x(),
                                               shapes[j].part(i).y()),
                                       1, cv::Scalar(0, 0, 255), -1);
                            if (load == false) {
                                myfile << shapes[j].part(i).x() << " "
                                       << shapes[j].part(i).y() << endl;
                            }
                        }
                        // Parameter description Image center line width color
                        // line type Display number cv::putText(temp,
                        // to_string(i), cvPoint(shapes[0].part(i).x(),
                        // shapes[0].part(i).y()), cv::FONT_HERSHEY_PLAIN, 1,
                        // cv::Scalar(0, 0, 255));
                    }
                    if (load == false) {
                        myfile.close();
                ...
                int height_left_eye1 =
                    y_41 - y_37; // Longitudinal distance from 37 to 41
                // cout << "Left Eye Height 1\t" << height_left_eye1 << endl;
                int height_left_eye2 =
                    y_40 - y_38; // Longitudinal distance from 38 to 40
                // cout << "Left Eye Height 2\t" << height_left_eye2 << endl;
                float height_left_eye = (height_left_eye1 + height_left_eye2) /
                                        2; // up and down distance of eyes
                // cout << "Left Eye Height\t" << height_left_eye << endl;
                int length_left_eye = x_39 - x_36;
                // cout << "Left Eye Length\t" << length_left_eye << endl;
                if (height_left_eye ==
                    0) // When the eyes are closed, the distance may be detected
                       // as 0 and the aspect ratio is wrong
                    height_left_eye = 1;
                ...
                unsigned int height_right_eye1 =
                    y_47 - y_43; // Longitudinal distance from 37 to 41
                unsigned int height_right_eye2 =
                    y_46 - y_44; // Longitudinal distance from 38 to 40
                float height_right_eye =
                    (height_right_eye1 + height_right_eye2) /
                    2; // up and down distance of eyes
                if (height_right_eye ==
                    0) // When the eyes are closed, the distance may be detected
                       // as 0 and the aspect ratio is wrong
                ...
                // Take the average aspect ratio of the two eyes as the aspect
                // ratio of the eyes
                float EAR_eyes = (EAR_left_eye + EAR_right_eye) / 2;

                // cout << "The aspect ratio of the eyes is" << EAR_eyes <<
                // endl;

                // Draw the waveform of the eye
                eye_now_x =
                    eye_now_x + 1; // abscissa (one point for every 10 images)
                eye_now_y = 900 - (EAR_eyes * 900); // Vertical coordinate
                Point poi1 =
                    Point(eye_previous_x, eye_previous_y); // previous point
                Point poi2 = Point(eye_now_x, eye_now_y);  // Current point
                Scalar eyes_color = Scalar(0, 255, 0);
                cv::line(Eye_Waveform, poi1, poi2, eyes_color, 1,
                         LINE_AA); // Draw a line
                ...
                if (blink_EAR_before > 0.2 && blink_EAR_now <= 0.2 &&
                    blink_EAR_after > 0.2) {
                    ...
                    //                        }
                    //                        result << "Blink at: " << k << "
                    //                        Closest marked blink: " <<
                    //                        (closestLeft) << " " <<
                    //                        (closestRight) << endl; cout <<
                    //                        "Blink at: "
                    //                        << k << " Closest marked blink: "
                    //                        << (closestLeft) << " " <<
                    //                        (closestRight) << endl;
                    //
                    //                        myfile.close();
                ...
                _gcvt_s(count_blink_text, count_blink,
                        10); // Convert hight_left_eye from float type to string
                             // type

                putText(temp, count_blink_text, Point(10, 100),
                        FONT_HERSHEY_COMPLEX, 1.0, Scalar(0, 0, 255), 1,
                        LINE_AA);
            }
        ...
        cout << "You need dlib‘s default face landmarking file to run this "
                "example."
             << endl;
...
int main3() {
    ...
    for (int i = 0;; i++) {
        ...
        if (faceDetected != bool(faces)) {
            ...
            output << "v snímke " << i << " " << faceDetectionStrings[faces]
                   << "detegovana tvar" << endl;
        }
...
int main1() {
    ...
    while (true) {
        ...
        if (bufSize > 0)         // if some
        {
            ...
            // an alternative is to unlock after processing (this will lock
            // grabbing)
            buffer.front().copyTo(
                frame);   // get the oldest grabbed frame (queue=FIFO)
        ...
        // if bufSize is increasing means that process time is too slow regards
        // to grab time may be you will have out of memory soon
        if (bufSize)
