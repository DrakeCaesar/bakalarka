...
Pattern Recognition, Volume 47, Issue 11, November 2014, Pages 3693-3708, ISSN
0031-3203, http://dx.doi.org/10.1016/j.patcog.2014.05.012.
(http://www.sciencedirect.com/science/article/pii/S0031320314001976)


The comments in the code refer to the abovementioned paper.
If you need further details about the code or the algorithm, please contact me
at:
...
CEllipseDetectorYaed::CEllipseDetectorYaed(void)
    : _times(6, 0.0), _timesHelper(6, 0.0) {
...
void CEllipseDetectorYaed::SetParameters(
    Size szPreProcessingGaussKernelSize, double dPreProcessingGaussSigma,
...
void CEllipseDetectorYaed::GetFastCenter(vector<Point> &e1, vector<Point> &e2,
...

    {
        ...
        for (uint ii = 0; ii < minPoints; ++ii) {
            ...
            med.push_back(
                Point2f((e1[j].x + x1) * 0.5f, (e1[j].y + y1) * 0.5f));
        }
...

    {
        ...
        for (uint ii = 0; ii < minPoints; ++ii) {
            ...
            med.push_back(
                Point2f((e2[j].x + x1) * 0.5f, (e2[j].y + y1) * 0.5f));
        }
...
void CEllipseDetectorYaed::DetectEdges13(Mat1b &DP, VVP &points_1,
                                         VVP &points_3) {
...
void CEllipseDetectorYaed::DetectEdges24(Mat1b &DN, VVP &points_2,
                                         VVP &points_4) {
    // Vector of connected edge points
    VVP contours;
...
void CEllipseDetectorYaed::FindEllipses(Point2f &center, VP &edge_i, VP &edge_j,
                                        VP &edge_k, EllipseData &data_ij,
                                        EllipseData &data_ik,
        ...
        for (int ij1 = 0; ij1 < sz_ij1; ++ij1) {
            ...
            for (int ik1 = 0; ik1 < sz_ik1; ++ik1) {
                ...
                if (Np > 1.f) {
                    Np = 1.f / Np;
                    rhoDeg =
                        cvRound((rho * 180 / CV_PI) + 180) % 180; // [0,180)
                } else {
                ...
                if (0 <= iNp && iNp < ACC_N_SIZE && 0 <= rhoDeg &&
                    rhoDeg < ACC_R_SIZE) {
    ...
    for (ushort l = 0; l < sz_ei; ++l) {
        ...
        float Ax =
            sqrt((x0 * x0 * Np * Np + y0 * y0) / ((Np * Np) * (1.f + Kp * Kp)));
    ...
    // limitations. The main idea is that for a given ellipse (TD) even if the
    // score is high, the arcs can cover only a small amount of the contour of
    // the estimated ellipse. A low reliability indicate that the arcs form an
    // elliptic shape by chance, but do not underlie an actual ellipse. The
    // value is normalized between 0 and 1. The default value is 0.4.

    // It is somehow similar to the "Angular Circumreference Ratio" saliency
    // criteria as in the paper: D. K. Prasad, M. K. Leung, S.-Y. Cho, Edge
    // curvature and convexity based ellipse detection method, Pattern
    // Recognition 45 (2012) 3204-3221.
...
Point2f CEllipseDetectorYaed::GetCenterCoordinates(EllipseData &data_ij,
                                                   EllipseData &data_ik) {
    float xx[7];
    float yy[7];
    ...
    for (ushort i = 0; i < sz_i; ++i) {
        ...
        for (ushort j = 0; j < sz_j; ++j) {
            ...
            for (ushort k = 0; k < sz_k; ++k) {
                ...
                // satisfied, we can start estimating the ellipse parameters
                ...
                FindEllipses(center, edge_i, edge_j, edge_k, data_ij, data_ik,
                             ellipses);
            }
        }
    }
...
void CEllipseDetectorYaed::PrePeocessing(Mat1b &I, Mat1b &DP, Mat1b &DN) {

    Tic(0); // edge detection

    // Smooth image
    GaussianBlur(I, I, _szPreProcessingGaussKernelSize,
                 _dPreProcessingGaussSigma);
...
void CEllipseDetectorYaed::ClusterEllipses(vector<Ellipse> &ellipses) {
    ...
    for (int i = 1; i < iNumOfEllipses; ++i) {
        ...
        for (int j = 0; j < sz_clusters; ++j) {
            ...
            // Centers
            float Dc = ((e1._xc - e2._xc) * (e1._xc - e2._xc) +
                        (e1._yc - e2._yc) * (e1._yc - e2._yc));
            if (Dc > th_Dc) {
                // not same cluster
                continue;
            }
            ...
            if ((Dr > th_Dr) && (ba_e1 < th_Dr_circle) &&
                (ba_e2 < th_Dr_circle)) {
                // not same cluster
                continue;
            }
...
void CEllipseDetectorYaed::DrawDetectedEllipses(Mat3b &output,
    ...
    for (int i = 0; i < n; ++i) {
        ...
        ellipse(output, Point(cvRound(e._xc), cvRound(e._yc)),
                Size(cvRound(e._a), cvRound(e._b)), e._rad * 180.0 / CV_PI, 0.0,
                360.0, color, thickness);
    }
}