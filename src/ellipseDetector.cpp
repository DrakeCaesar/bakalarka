...
// Should be checked
void SaveEllipses(
    const string &workingDir, const string &imgName,
    const vector<Ellipse> &ellipses /*, const vector<double>& times*/) {
...
// Should be checked !!!!!
std::tuple<float, float, float> Evaluate(const vector<Ellipse> &ellGT,
                                         const vector<Ellipse> &ellTest,
                                         const float th_score,
                                         const Mat3b &img) {
    ...
    for (unsigned i = 0; i < sz_gt; ++i) {
        ...
        ellipse(tmp, Point(e._xc, e._yc), Size(e._a, e._b),
...
Mat OnImage(Mat matimage) {
    ...
    yaed->SetParameters(szPreProcessingGaussKernelSize,
                        dPreProcessingGaussSigma, fThPos, fMaxCenterDistance,
    ...
    for (unsigned i = 0; i < gt.size(); ++i) {
        ...
        ellipse(resultImage, Point(cvRound(e._xc), cvRound(e._yc)),
...
void OnVideo() {
    ...
    int iNs = 16;
    float fMaxCenterDistance =
        sqrt(float(width * width + height * height)) * fTaoCenters;
    ...
    while (true) {
        ...
        vector<Ellipse> gt;
        LoadGT(gt, sWorkingDir + "/gt/" + "gt_" + imagename + ".txt",
               true); // Prasad is in radians
        ...
        for (unsigned i = 0; i < gt.size(); ++i) {
            ...
            Scalar color(0, 0, 255);
            ellipse(resultImage, Point(cvRound(e._xc), cvRound(e._yc)),
                    Size(cvRound(e._a), cvRound(e._b)), e._rad * 180.0 / CV_PI,
                    0.0, 360.0, color, 3);
        }
...
void OnDataset() {
    string sWorkingDir =
        "D:\\data\\ellipse_dataset\\Random Images - Dataset #1\\";
    // string sWorkingDir = "D:\\data\\ellipse_dataset\\Prasad Images - Dataset
    // Prasad\\";
    ...
    for (const auto &image_name : names) {
        ...
        int iNs = 16;
        float fMaxCenterDistance =
            sqrt(float(sz.width * sz.width + sz.height * sz.height)) *
            fTaoCenters;
        ...
        CEllipseDetectorYaed *yaed = new CEllipseDetectorYaed();
        yaed->SetParameters(
            szPreProcessingGaussKernelSize, dPreProcessingGaussSigma, fThPos,
        ...
        vector<Ellipse> gt;
        LoadGT(gt, sWorkingDir + "gt\\" + "gt_" + name_ext + ".txt",
               false); // Prasad is in radians,set to true
...
int main(int argc, char** argv)
{
        ...
        // _splitpath_s(resolved_path, NULL, 0, NULL, 0, NULL, 0, extension,
20);
