...
int inline sgn(float val) { return (0.f < val) - (val < 0.f); };
...
struct Ellipse {
    ...
    Ellipse(const Ellipse &other)
        : _xc(other._xc), _yc(other._yc), _a(other._a), _b(other._b),
          _rad(other._rad), _score(other._score){};

    void Draw(Mat &img, const Scalar &color, const int thickness) {
        ellipse(img, Point(cvRound(_xc), cvRound(_yc)),
                Size(cvRound(_a), cvRound(_b)), _rad * 180.0 / CV_PI, 0.0,
