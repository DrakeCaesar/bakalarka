...
void cvCanny2(const void *srcarr, void *dstarr, double low_thresh,
              double high_thresh, void *dxarr, void *dyarr, int aperture_size) {
    // cv::Ptr<CvMat> dx, dy;
...
void Canny2(InputArray image, OutputArray _edges, OutputArray _sobel_x,
            OutputArray _sobel_y, double threshold1, double threshold2,
    ...
    cvCanny2(&c_src, &c_dst, threshold1, threshold2, &c_dx, &c_dy,
...
void LabelingRect(Mat1b &image, VVP &segments, int iMinLength,
                  vector<Rect> &bboxes) {
        ...
        for (x = 0; x < w; x++) {
            if ((src(y, x)) != 0) // punto non etichettato: seme trovato
            {
                ...
                if (sp3 >= iMinLength) {
                    ...
                    bboxes.push_back(
                        Rect(Point(iMinx, iMiny), Point(iMaxx + 1, iMaxy + 1)));
                    segments.push_back(component);
                }
            }
        }
...
void cvCanny3(const void *srcarr, void *dstarr, void *dxarr, void *dyarr,
              int aperture_size) {
    // cv::Ptr<CvMat> dx, dy;
    cv::AutoBuffer<char> buffer;
    std::vector<uchar *> stack;
