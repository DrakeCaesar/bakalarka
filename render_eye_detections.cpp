...
namespace dlib
{
    ...
    )
    {
        ...
        {
            DLIB_CASSERT(dets[i].num_parts() == 12,
                "\t std::vector<image_window::overlay_line> render_eye_detections()"
            ...
            const full_object_detection& d = dets[i];
                ...
                lines.push_back(image_window::overlay_line(d.part(0), d.part(5), color));

                // Right eye
                for (unsigned long i = 7; i <= 11; ++i)
                    lines.push_back(image_window::overlay_line(d.part(i), d.part(i-1), color));
            lines.push_back(image_window::overlay_line(d.part(6), d.part(11), color));
