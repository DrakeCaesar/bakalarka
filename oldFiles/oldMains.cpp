      ...
      if (!shapes.empty()) {
        ...
        for (int j = 0; j < faceNumber; j++) {
          ...
          for (int i = 0; i < 68; i++) {
            ...
            if (load == false) {
              cv::circle(temp,
                         cvPoint(shapes[j].part(i).x(), shapes[j].part(i).y()),
                         1, cv::Scalar(0, 0, 255), -1);
              if (load == false) {
                myfile << shapes[j].part(i).x() << " " << shapes[j].part(i).y()
                       << endl;
              }
            }
        ...
        float height_right_eye = (height_right_eye1 + height_right_eye2) /
                                 2; // up and down distance of eyes
        if (height_right_eye ==
            0) // When the eyes are closed, the distance may be detected
               // as 0 and the aspect ratio is wrong
        ...
        putText(temp, count_blink_text, Point(10, 100), FONT_HERSHEY_COMPLEX,
                1.0, Scalar(0, 0, 255), 1, LINE_AA);
      }
